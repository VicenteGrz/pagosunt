...
qrcode.callback = (respuesta) => {
  if (respuesta) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        if (resultado) {
          ...
          tabla.classList.add("table"); // Agregar clases de Bootstrap para estilos (opcional)
          
          // Crear la cabecera de la tabla
          const cabecera = tabla.createTHead();
          const filaCabecera = cabecera.insertRow();
          const camposMostrar = ['nombre', 'telefono', 'correo', 'plantel']; // Campos a mostrar
    ...
    // Envia el valor del código QR al servidor
